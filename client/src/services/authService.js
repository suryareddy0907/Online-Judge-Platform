...
export const getPublicProblems = async (params = {}) => {
  ...
  const response = await fetch(`${import.meta.env.VITE_API_URL}/api/problems?${queryParams}`);
  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.message || 'Something went wrong');
  }
...
export const getMySubmissions = async (params = {}) => {
  const token = localStorage.getItem('token');
  const queryParams = new URLSearchParams(params);
  const response = await fetch(`${import.meta.env.VITE_API_URL}/api/problems/my-submissions?${queryParams}`, {
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`,
    },
...
export const getPublicStats = async () => {
  ...
  const response = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/public-stats`, { headers });
  if (!response.ok) {
    const error = await response.json();
...
export const getLeaderboard = async (params = {}) => {
  const queryParams = new URLSearchParams(params);
  const response = await fetch(`${import.meta.env.VITE_API_URL}/api/leaderboard?${queryParams}`);
  if (!response.ok) {
    const error = await response.json();
...
export const getRecentActivity = async (params = {}) => {
  const queryParams = new URLSearchParams(params);
  const response = await fetch(`${import.meta.env.VITE_API_URL}/api/leaderboard/activity?${queryParams}`);
  if (!response.ok) {
    const error = await response.json();
...
export const getUserSolvedCount = async (userId) => {
  const response = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/user/${userId}/solved-count`);
  if (!response.ok) {
    const error = await response.json();
...
export const getPublicContests = async (params = {}) => {
  ...
  const response = await fetch(`${import.meta.env.VITE_API_URL}/api/contests?${queryParams}`, { headers });
  if (!response.ok) {
    const error = await response.json();
...
export const registerForContest = async (contestId) => {
  const token = localStorage.getItem('token');
  const response = await fetch(`${import.meta.env.VITE_API_URL}/api/contests/${contestId}/register`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`,
